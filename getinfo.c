...
/**
 * set_info - inisilizes info_t struct
 * @info: struct adress
 * @av: arguments
 *
 ...
 * @info: struct address
 * @all: true if freeing all feilds
 */
...
void free_info(info_t *info, int all)
{
	...
	if (info->right_redirect_to_fd > 2)
	{
		...
		info->right_redirec_to_fd = -1;
	}
	if (info->heredoc_txt && info->_left_redirect_from_fd != HEREDOC_FD)
